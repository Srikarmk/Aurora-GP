...
class RegionIdentifier:
    ...
    def predict_regions(self, X_test):
        """
        ...
        # Assign regions
        regions = np.zeros(len(X_test), dtype=int)
        regions[test_importance >= self.high_threshold_value] = 2  # High
        regions[test_importance <= self.low_threshold_value] = 0   # Low
        regions[(test_importance > self.low_threshold_value) & 
                (test_importance < self.high_threshold_value)] = 1  # Medium
